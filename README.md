...
| `Seawater_Carbon/` | `carbon_sw_eq.py`, `constants.py`, `constants_P.py`, `ReadMeFirst.md` | Calculates carbonate-system equilibrium in seawater and prints pH and dissolved carbon species. The `constants.py` and `constants_P.py` files provide equilibrium-constant functions used by the main carbonate-equilibrium script, and `ReadMeFirst.md` gives folder-specific notes. |
...
For the seawater carbonate-system calculation:

```bash
cd Seawater_Carbon
python carbon_sw_eq.py
```
The main script uses equilibrium-constant functions provided in `constants.py` and `constants_P.py`. See `ReadMeFirst.md` for notes specific to this folder.
## Data files
